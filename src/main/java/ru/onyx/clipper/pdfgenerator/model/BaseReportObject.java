...
public abstract class BaseReportObject {
    ...
    protected void Load(Node node) {
        ...
        if (attrObj != null) {
            ...
            marginLeft = Float.parseFloat(parseAttribute(attrObj, Report.marginleft, Report.margin_value));
            marginTop = Float.parseFloat(parseAttribute(attrObj, Report.margintop, Report.margin_value));
            marginRight = Float.parseFloat(parseAttribute(attrObj, Report.marginright, Report.margin_value));
            ...
            if (nodeName.equals("items")) {
                ...
                switch (nodeName) {
                    ...
                    case Report.newpage:
                        ...
                        break;
                    case Report.newsection:
                        items.add(new ReportNewSection(item));
                        break;
                    case Report.ifcondition:
                        NodeList ifStatementChildren = nodes.item(i).getChildNodes();
                        ReportConditionalStatements.parseIfStatement(ifStatementChildren, pGetter, Report.logicalcondition, Report.elsecondition, Report.paragraph, items, fonts,report);
                }
            }
        }
    }
